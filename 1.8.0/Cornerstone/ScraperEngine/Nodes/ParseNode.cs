...
namespace Cornerstone.ScraperEngine.Nodes {
    ...
    public class ParseNode : ScraperNode {
        ...
        public override void Execute(Dictionary<string, string> variables) {
            if (Context.DebugMode) logger.Debug("executing parse: " + xmlNode.OuterXml);
        ...
        private void processPattern(Dictionary<string, string> variables, string parsedInput, string parsedName) {
            ...
            if (Context.DebugMode) logger.Debug("name: " + parsedName + " ||| pattern: " + parsedPattern + " ||| input: " + (parsedInput != null && parsedInput.Length < 500 ? parsedInput : "[not logged due to size]"));
            ...
            if (matches.Count == 0) {
                if (Context.DebugMode) logger.Debug("Parse node returned no results... " + xmlNode.OuterXml);
                return;
            }
